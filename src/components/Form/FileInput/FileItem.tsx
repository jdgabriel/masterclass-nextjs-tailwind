...
import { CheckCircle2, CircleSlash, Trash2, UploadCloud } from 'lucide-react'
...
const fileItem = tv({
  ...
  variants: {
    state: {
      ...
      complete: {
        container: 'border-violet-500 dark:border-violet-500/20',
      },
      error: {
        container:
          'bg-error-25 border-error-300 dark:bg-error-500/5 dark:border-error-500/30',
        icon: 'border-error-50 bg-error-100 text-error-600 dark:bg-error-500/5 dark:border-error-500/5',
        deleteButton:
...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  return (
    <div className={container()}>
      <div className={icon()}>
        {state === 'error' ? (
          <CircleSlash className="dark:text-error-500/50 h-4 w-4" />
        ) : (
